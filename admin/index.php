...
$msgErrorsCateg=$msgErrorsDesc=$msgErrorsProd="";    
    ...
    $target_dir = $target_file ="";
     if( isset($_POST['adaugare']) ){
			...
			$imgData = pathinfo($_FILES['fileToUpload']["name"]);
			$imgname = $imgData['filename'] . "_" . time() . "." . $imgData['extension'];
        ...
        if(empty($nume_produs)){
         ...
         if($uploadOk && empty($msgErrorsCateg) && empty($msgErrorsDesc) && empty($msgErrorsProd) ) {

            $sql= "INSERT INTO `produse` (`nume_produs`,`imagine`,`categorie`, `descriere`)
                    VALUES ( '$nume_produs','$imgname','$categorie','$descriere' )";
       ...
       if( isset($_GET['id']) && $_GET['id'] != 0){
           ...
           $getImg = "SELECT imagine FROM produse WHERE id=" . $_GET['id'];
		   
		   $rq = mysqli_query($conn, $getImg);
		   
		   $imgName = null;
		   if(!mysqli_error()){
			   $rw = mysqli_fetch_assoc($rq);
			   $imgName = $rw['imagine'];
		   }
		   ...
		   if(mysqli_query($conn,$delete)){
			   ...
			   //echo "File to delete:" . UPLOAD_DIR . $imgName;
